...
.then(response => {
    ...
    let content = `
        <div class="card my-5">
            ...
            <div class="card-body">
                ...
                <div class="card p-2">
                    <div class="d-flex">
                        <a class="navbar-brand" href="#"><img class="rounded-circle mx-2" style="width: 30px" src="${response.data.data.comments[0].author.profile_image}"></a>
                        <b>${response.data.data.comments[0].author.username}</b>
                    </div>
                    <p class="m-2">${response.data.data.comments[0].body}</p>
                </div>
            </div>
        </div>        
    `
    ...
    // console.log(response.data.data.comments[0].author)
})
.catch(error => {
console.log(error)
})